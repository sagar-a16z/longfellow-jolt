...
use tracer::{
    ...
    instruction::{
        ...
        sd::SD, sltu::SLTU, virtual_advice::VirtualAdvice,
        virtual_assert_eq::VirtualAssertEQ, Instruction,
    },
...
impl P256Mulq {
    ...
    fn inline_sequence(mut self) -> Vec<Instruction> {
        ...
        if self.is_scalar_field {
            ...
            self.asm.emit_u::<LUI>(*self.p1, P256_PN[0]);
            self.asm.emit_u::<LUI>(*self.p2, P256_PN[1]);
            self.asm.emit_u::<LUI>(*self.p3, P256_PN[3]);
        } else {
            // Base field: p = [1, 0xFFFFFFFF00000000, 0xFFFFFFFFFFFFFFFF, 0x00000000FFFFFFFE]
        ...
        match self.op_type {
            MulqType::Div => {
                ...
                self.asm
                    .emit_b::<VirtualAssertEQ>(*self.r[0], *self.aux, 0);
            }
            _ => {
            ...
            if self.is_scalar_field {
                ...
                if k < 4 {
                    self.mac_low_conditional(
                        !first, rk_next, rk, *self.w[k], *self.p1, *self.aux,
                    );
                    first = false;
                }
                ...
                if k >= 1 && k - 1 < 4 {
                    self.mac_low_conditional(
                        !first, rk_next, rk, *self.w[k - 1], *self.p2, *self.aux,
                    );
                    first = false;
                }
                ...
                if k >= 3 && k - 3 < 4 {
                    self.mac_low_conditional(
                        !first, rk_next, rk, *self.w[k - 3], *self.p3, *self.aux,
                    );
                    first = false;
                }
                ...
                if k >= 1 && k - 1 < 4 {
                    self.mac_high_conditional(
                        !first, rk_next, rk, *self.w[k - 1], *self.p1, *self.aux,
                    );
                    first = false;
                }
                ...
                if k >= 2 && k - 2 < 4 {
                    self.mac_high_conditional(
                        !first, rk_next, rk, *self.w[k - 2], *self.p2, *self.aux,
                    );
                    first = false;
                }
                ...
                if k >= 4 && k - 4 < 4 {
                    self.mac_high_conditional(
                        !first, rk_next, rk, *self.w[k - 4], *self.p3, *self.aux,
                    );
                    first = false;
                }
            } else {
                ...
                if k >= 2 && k - 2 < 4 {
                    self.mac_high_conditional(
                        !first, rk_next, rk, *self.w[k - 2], *self.p1, *self.aux,
                    );
                    first = false;
                }
                ...
                if k >= 3 && k - 3 < 4 {
                    self.mac_high_conditional(
                        !first, rk_next, rk, *self.w[k - 3], *self.p2, *self.aux,
                    );
                    first = false;
                }
...
pub fn p256_mulq_sequence_builder(
    asm: InstrAssembler,
    operands: FormatInline,
) -> Vec<Instruction> {
    let builder = P256Mulq::new(asm, operands, MulqType::Mul, false);
    builder.inline_sequence()
}

/// Custom trace function for P-256 base field modular multiplication
pub fn p256_mulq_advice(
    asm: InstrAssembler,
    operands: FormatInline,
    cpu: &mut Cpu,
) -> VecDeque<u64> {
...
/// Virtual instruction builder for P-256 base field modular squaring
pub fn p256_squareq_sequence_builder(
    asm: InstrAssembler,
    operands: FormatInline,
) -> Vec<Instruction> {
    let builder = P256Mulq::new(asm, operands, MulqType::Square, false);
    builder.inline_sequence()
}

/// Custom trace function for P-256 base field modular squaring
pub fn p256_squareq_advice(
    asm: InstrAssembler,
    operands: FormatInline,
    cpu: &mut Cpu,
) -> VecDeque<u64> {
...
/// Virtual instruction builder for P-256 base field modular division
pub fn p256_divq_sequence_builder(
    asm: InstrAssembler,
    operands: FormatInline,
) -> Vec<Instruction> {
    let builder = P256Mulq::new(asm, operands, MulqType::Div, false);
    builder.inline_sequence()
}

/// Custom trace function for P-256 base field modular division
pub fn p256_divq_advice(
    asm: InstrAssembler,
    operands: FormatInline,
    cpu: &mut Cpu,
) -> VecDeque<u64> {
...
/// Virtual instruction builder for P-256 scalar field modular multiplication
pub fn p256_mulr_sequence_builder(
    asm: InstrAssembler,
    operands: FormatInline,
) -> Vec<Instruction> {
    let builder = P256Mulq::new(asm, operands, MulqType::Mul, true);
    builder.inline_sequence()
}

/// Custom trace function for P-256 scalar field modular multiplication
pub fn p256_mulr_advice(
    asm: InstrAssembler,
    operands: FormatInline,
    cpu: &mut Cpu,
) -> VecDeque<u64> {
...
/// Virtual instruction builder for P-256 scalar field modular squaring
pub fn p256_squarer_sequence_builder(
    asm: InstrAssembler,
    operands: FormatInline,
) -> Vec<Instruction> {
    let builder = P256Mulq::new(asm, operands, MulqType::Square, true);
    builder.inline_sequence()
}

/// Custom trace function for P-256 scalar field modular squaring
pub fn p256_squarer_advice(
    asm: InstrAssembler,
    operands: FormatInline,
    cpu: &mut Cpu,
) -> VecDeque<u64> {
...
/// Virtual instruction builder for P-256 scalar field modular division
pub fn p256_divr_sequence_builder(
    asm: InstrAssembler,
    operands: FormatInline,
) -> Vec<Instruction> {
    let builder = P256Mulq::new(asm, operands, MulqType::Div, true);
    builder.inline_sequence()
}

/// Custom trace function for P-256 scalar field modular division
pub fn p256_divr_advice(
    asm: InstrAssembler,
    operands: FormatInline,
    cpu: &mut Cpu,
) -> VecDeque<u64> {
